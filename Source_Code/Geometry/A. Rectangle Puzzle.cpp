...
int main(){
    ...
    w/=2.0; h/=2.0; alpha=convdr(alpha);

    quad prev,now;

    ps[0]=point(-w,-h); ps[1]=point(w,-h); ps[2]=point(w,h); ps[3]=point(-w,h);
    prev=quad(ps[0],ps[1],ps[2],ps[3]);

    psr[0]=rotate2D(ps[0],alpha); psr[1]=rotate2D(ps[1],alpha);
    psr[2]=rotate2D(ps[2],alpha); psr[3]=rotate2D(ps[3],alpha);

    int count1=0;
    for(i=0;i<4;i++){
        if(inside_quad(prev,psr[i])){
           p[count1++]=psr[i];
        }
    }

    segment lines[5],linesr[5];

    lines[0]=segment(ps[0],ps[1]); lines[1]=segment(ps[1],ps[2]);
    lines[2]=segment(ps[2],ps[3]); lines[3]=segment(ps[3],ps[0]);

    linesr[0]=segment(psr[0],psr[1]); linesr[1]=segment(psr[1],psr[2]);
    linesr[2]=segment(psr[2],psr[3]); linesr[3]=segment(psr[3],psr[0]);

    for(i=0;i<4;i++){
        for(j=0;j<4;j++){
            point p1;
            if(intersection(lines[i],linesr[j],p1)){
                p[count1++]=p1;
            }
        }
    }
    ...
    convex_hull(c,count1,no);

	double ans=areaPoly(c,no);
