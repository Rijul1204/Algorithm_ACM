...
struct node{
    ...
    double cost;
    node(){}
    node(int _ind,int _pre,double _cost){
        ind=_ind; pre=_pre; cost=_cost;
    }
};
...
int main(){
    ...
    while(test--){
        ...
        for(i=1;i<=n;i++){
            ...
            v[k].push_back(l);
            v[l].push_back(k);
            double dis=distancepp(p[k],p[l])*2.0;
            w[k].push_back(dis);
            w[l].push_back(dis);
        }

        double ans=-1.000,ans1;

        for(i=1;i<=c;i++){
			if(p[i].y<cen.y){
				ans1=dij(i);
				if(iseq(ans,(-1.0))) ans=ans1;
				if(iseq(ans1,(-1.0))) continue;
...
double dij(int s){
    ...
    while(!pq.empty()){
        ...
        for(j=0;j<v[i].size();j++){
            k=v[i][j];
            if(k==tmp.pre) continue;
            if(p[k].y<p[s].y || (iseq(p[k].y,p[s].y) && p[k].x<p[s].x) ) continue;
            if((tmp.pre!=tmp.ind) && (cross(p[tmp.pre],p[tmp.ind],p[k])>0)) continue;

            if(cross(cen,p[i],p[k])<0){
                tmp1=node(k,tmp.ind,tmp.cost+w[i][j]);
                if((tmp1.cost>dist[s][k]) && !iseq(dist[s][k],(-1.0))) continue;
                dist[s][k]=tmp1.cost;
                pq.push(tmp1);
            }
        }
