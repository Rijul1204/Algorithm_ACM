...
int main(){
    ...
    while(test--){
        ...
        double slope_prev=0.0,slope_now,ans=0;
        line now,prev=line(piv,p[0]);
        point pivp;

        for(i=1;i<n;i++){
            now=line(p[i],p[i-1]);
            if(!intersection(now,prev,pivp)) continue;
            if(!inside_segment(segment(p[i],p[i-1]),pivp)) continue;
            slope_now=cal_slope(p[i],p[i-1]);
            if(slope_now<slope_prev){
            }
            else{
                ans+=distancepp(pivp,p[i]);
                prev=line(piv,p[i]);
                slope_prev=cal_slope(p[i],piv);
            }
        }
